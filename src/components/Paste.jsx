...
function Paste() {
  ...
  return (
    <div>
      ...
      />
      <div
        className="w-[65vw] display-flex flex-col gap-2 mt-3 p-3 border border-gray-300 rounded-xl 
         focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500
         resize-none "
      >
        {
        filterdData.length > 0 &&
          filterdData.map((paste, index) => {
            return (
              <div
                className="bg-white shadow-md rounded-2xl min-h-[20vh] flex mt-2 mb-2 p-4 border border-gray-200 
                hover:shadow-lg transition-shadow duration-200 
                w-full max-w-[100]"
                key={paste.id + "-" + index}
              >
                <div className="flex flex-col max-w-[80%] min-w-[70%]">
                  <div className="text-lg text-left ml-1 mr-1 pl-2 border-1 font-semibold bg-blue-200 text-emerald-950 break-words">
                    Title:{paste.title}
                  </div>
                  <div className="text-sm text-left text-blue-400 break-words ">
